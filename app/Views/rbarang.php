...
<style>
  ...
  .table td.actions {
    width: 150px; /* Adjust width for action buttons */
  }
  .btn-edit {
  ...
  <tbody>
    <?php $no = 1; foreach ($satu as $dua): ?>
      <tr>
        ...
        <td><?= $dua->status ?></td>
        <td class="actions">
         <a href="<?= base_url('home/restore/' . $dua->id_barang) ?>" class="btn btn-warning">Restore</a>
          <a href="<?= base_url('home/hapus_permanen/' . $dua->id_barang) ?>" class="btn-delete" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">Hapus</a>
        </td>
      </tr>
    <?php endforeach; ?>
  </tbody>
